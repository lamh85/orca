...
const wordsList = fullText.replace(/\./g, '').split(' ')

const randomIndices = () => {
  const resultWordCount = Math.ceil(Math.random() * 9) + 1
  
  let counter = 1
  const result = []
  const ipsumLength = wordsList.length
  const ipsumMaxIndex = ipsumLength - 1

  while (counter <= resultWordCount) {
    const randomIndex = Math.round(Math.random() * ipsumMaxIndex)
...
export const randomWords = customIndices => {
  // TODO: test if argument's default value can be a function result
